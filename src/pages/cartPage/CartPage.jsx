...
import emptycart from '../../components/Navbar/images/first_white_girl_drbl.gif'
...
// for increase qty 
  ...
  if(cartData.length==0){
    return(
      <div className={styles.empty}>Cart Is Empty <button onClick={()=>Navigate("/")}>Shop Now</button><img src={emptycart}/></div>
    )
  }
  return (
    ...
    {cartData?.map((el,i)=>(
      ...
      <div><button  onClick={()=>decreaseQty(el)}>-</button>{+el.quantity}<button onClick={()=>increaseQty(el)}>+</button></div>
   ...
   <hr/>
    <div className={styles.total_price}><p>Total Amount</p> <p>₹ {totalPrice}</p></div>
    </div>
  )
}
